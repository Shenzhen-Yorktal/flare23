    ...
    for idx in range(len(list_of_lists)):
        ...
        with torch.no_grad():
            ...
            if np.any([i != j for i, j in zip(np.array(current_shape[1:]), np.array(size_ac))]):
                prob_map = np.zeros([softmax.shape[0], *size_ac])
                for i in range(softmax.shape[0]):
                    plane = softmax[i:i+1][None].half()
                    prob_map[i] = f.interpolate(plane.cuda(), mode='trilinear', size=size_ac, align_corners=False)[0]
                seg_old_spacing = slice_argmax(prob_map)
                del prob_map
                torch.cuda.empty_cache()
            else:
            ...
            if np.any([i != j for i, j in zip(np.array(current_shape[1:]), np.array(size_ac))]):
                ...
                for i in range(softmax.shape[0]):
                    plane = softmax[i:i + 1][None].half()
                    prob_map[i] = f.interpolate(plane.cuda(), mode='trilinear', size=size_ac, align_corners=False)[
                        0]
                seg_old_spacing2 = slice_argmax(prob_map)
                del prob_map
            else:
